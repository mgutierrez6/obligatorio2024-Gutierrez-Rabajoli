...
public class MyLinkedListImpl<T> implements MyList<T>, MyQueue<T>, MyStack<T> {
    ...
    private void addToBeginning(T value) {
        if (value != null) {
            ...
            if (this.first == null) { // verificamos si la lista es vacia, si lo es, significa que el elemento a agregar es el primero y el ultimo

                this.first = elementToAdd;
                this.last = elementToAdd;

            } else { // si la lista no es vacia, se agrega al principio
            ...
            // si el elemento a agregar es vacio se ignora
        }
    }
